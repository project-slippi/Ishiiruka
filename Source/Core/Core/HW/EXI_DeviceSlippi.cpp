...
void initDefaultPlaybackCodes(std::vector<u8> &list) {
	// Do nothing if list already initialized
	if (!list.empty())
		return;

	WARN_LOG(SLIPPI, "Initializing default playback codes list");

	std::string defaultCodeData;
	std::string _defaultCodeFilename = File::GetSysDirectory() + DEFAULT_PLAYBACK_CODES;
	if (!File::ReadFileToString(_defaultCodeFilename, defaultCodeData))
	{
		ERROR_LOG(SLIPPI, "Could not load default codes because default-playback-codes.bin was missing.");
		return;
	}

	// Initialize code list
	for (size_t i = 0, e = defaultCodeData.length(); i < e; ++i)
		list.push_back(defaultCodeData[i]);

	// Add the termination sequence
	list.insert(list.end(), {0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00});

	WARN_LOG(SLIPPI, "Default playback code list initialized with size: %d", list.size());
}

void CEXISlippi::prepareGeckoList()
{
	// Initialize default code list if it hasn't been yet
	static std::vector<u8> defaultCodeList;
	initDefaultPlaybackCodes(defaultCodeList);
