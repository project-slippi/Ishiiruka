...
void CEXISlippi::prepareOnlineMatchState()
{
	...
	auto delayFrames = (u8)SConfig::GetInstance().m_slippiOnlineDelay;
	if (SConfig::GetInstance().m_slippiOnlineAutoDelay && slippi_netplay)
	{
		WARN_LOG(SLIPPI_ONLINE, "Ping: %d", slippi_netplay->GetEnetPing());
	}
	m_read_queue.push_back(delayFrames);
