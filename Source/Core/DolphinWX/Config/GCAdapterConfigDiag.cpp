	...
	: wxDialog(parent, wxID_ANY, name), m_pad_id(tab_num)
{
	...
	wxCheckBox *const swap_y_and_z = new wxCheckBox(this, wxID_ANY, _("Swap Y and Z"));
	...
	swap_y_and_z->SetValue(SConfig::GetInstance().m_AdapterZYSwap[m_pad_id]);
	swap_y_and_z->Bind(wxEVT_CHECKBOX, &GCAdapterConfigDiag::OnZYSwap, this);
	...
	szr->AddSpacer(space5);
	szr->Add(swap_y_and_z, 0, wxEXPAND);
	szr->AddSpacer(space5);
...
void GCAdapterConfigDiag::OnZYSwap(wxCommandEvent &event) {
	SConfig::GetInstance().m_AdapterZYSwap[m_pad_id] = event.IsChecked();
}
